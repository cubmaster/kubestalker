...
import { KubeJob } from '../../../models/kubernetes.models';
  ...
  template: `
    <div class="resource-list">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="mb-0 text-muted">Jobs <span class="badge bg-secondary">{{ filtered.length }}</span></h6>
      ...
      <div class="table-responsive">
        <table class="table table-sm table-hover table-dark">
          <thead><tr><th>Name</th><th>Namespace</th><th>Completions</th><th>Duration</th><th>Age</th><th></th></tr></thead>
          <tbody>
            <tr *ngFor="let item of filtered" [class.table-danger]="item.hasError" (click)="openDrawer(item)" style="cursor:pointer">
              ...
              <td [class.text-danger]="(item.failed||0)>0">
                {{ item.succeeded||0 }}/{{ item.completions||1 }}
                <span *ngIf="(item.failed||0)>0" class="text-danger ms-1">({{ item.failed }} failed)</span>
              </td>
            ...
            <tr *ngIf="filtered.length===0&&!loading"><td colspan="6" class="text-center text-muted py-4">No jobs found</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
export class JobsComponent implements OnInit, OnDestroy {
  items:KubeJob[]=[]; searchText=''; loading=false; error:string|null=null;
  ...
  async load(){this.loading=true;this.error=null;try{this.items=await this.k8s.getJobs(this.ctx,this.ns)}catch(e:any){this.error=e.message}finally{this.loading=false}}
  ...
  openDrawer(item:KubeJob){this.drawer.open({resource:item.raw||item,resourceType:'Job',contextName:this.ctx,namespace:item.namespace,readOnly:true})}
  async deleteItem(item:KubeJob,e:Event){e.stopPropagation();if(!confirm(`Delete "${item.name}"?`))return;try{await this.k8s.deleteJob(this.ctx,item.namespace!,item.name);this.items=this.items.filter(i=>i!==item)}catch(err:any){this.error=err.message}}
}
