...
import { KubeStatefulSet } from '../../../models/kubernetes.models';
  ...
  template: `
    <div class="resource-list">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="mb-0 text-muted">StatefulSets <span class="badge bg-secondary">{{ filtered.length }}</span></h6>
      ...
      <div class="table-responsive">
        <table class="table table-sm table-hover table-dark">
          <thead>
            <tr><th>Name</th><th>Namespace</th><th>Ready</th><th>Service</th><th>Age</th><th></th></tr>
          </thead>
          <tbody>
            <tr *ngFor="let item of filtered" [class.table-danger]="item.hasError"
              ...
              <td><span [class.text-danger]="(item.readyReplicas||0)<(item.replicas||0)">{{ item.readyReplicas||0 }}/{{ item.replicas||0 }}</span></td>
              <td class="text-muted small">{{ item.serviceName }}</td>
            ...
            <tr *ngIf="filtered.length===0&&!loading"><td colspan="6" class="text-center text-muted py-4">No statefulsets found</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
export class StatefulSetsComponent implements OnInit, OnDestroy {
  items: KubeStatefulSet[] = []; searchText=''; loading=false; error:string|null=null;
  ...
  async load(){this.loading=true;this.error=null;try{this.items=await this.k8s.getStatefulSets(this.ctx,this.ns)}catch(e:any){this.error=e.message}finally{this.loading=false}}
  ...
  openDrawer(item:KubeStatefulSet){this.drawer.open({resource:item.raw||item,resourceType:'StatefulSet',contextName:this.ctx,namespace:item.namespace})}
  async deleteItem(item:KubeStatefulSet,e:Event){e.stopPropagation();if(!confirm(`Delete "${item.name}"?`))return;try{await this.k8s.deleteStatefulSet(this.ctx,item.namespace!,item.name);this.items=this.items.filter(i=>i!==item)}catch(err:any){this.error=err.message}}
}
