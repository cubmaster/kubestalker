...
import { KubeEndpoints } from '../../../models/kubernetes.models';
  ...
  template: `
    <div class="resource-list">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="mb-0 text-muted">Endpoints <span class="badge bg-secondary">{{ filtered.length }}</span></h6>
      ...
      <div class="table-responsive">
        <table class="table table-sm table-hover table-dark">
          <thead><tr><th>Name</th><th>Namespace</th><th>Endpoints</th><th>Age</th></tr></thead>
          <tbody>
            <tr *ngFor="let item of filtered" [class.table-danger]="item.hasError" (click)="openDrawer(item)" style="cursor:pointer">
              ...
              <td class="font-monospace small">{{ formatAddresses(item) }}</td>
              <td class="text-muted small">{{ getAge(item.creationTimestamp) }}</td>
            </tr>
            <tr *ngIf="filtered.length===0&&!loading"><td colspan="4" class="text-center text-muted py-4">No endpoints found</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
export class EndpointsComponent implements OnInit, OnDestroy {
  items:KubeEndpoints[]=[]; searchText=''; loading=false; error:string|null=null;
  ...
  async load(){this.loading=true;this.error=null;try{this.items=await this.k8s.getEndpoints(this.ctx,this.ns)}catch(e:any){this.error=e.message}finally{this.loading=false}}
  ...
  formatAddresses(ep:KubeEndpoints):string{return ep.addresses?.join(', ')||ep.subsets?.flatMap(s=>s.addresses?.map(a=>a.ip)||[]).join(', ')||'<none>'}
  openDrawer(item:KubeEndpoints){this.drawer.open({resource:item.raw||item,resourceType:'Endpoints',contextName:this.ctx,namespace:item.namespace,readOnly:true})}
}
