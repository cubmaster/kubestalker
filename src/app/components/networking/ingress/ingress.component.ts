...
import { KubeIngress } from '../../../models/kubernetes.models';
  ...
  template: `
    <div class="resource-list">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <h6 class="mb-0 text-muted">Ingresses <span class="badge bg-secondary">{{ filtered.length }}</span></h6>
        ...
        <table class="table table-sm table-hover table-dark">
          ...
          <tbody>
            <tr *ngFor="let item of filtered" [class.table-danger]="item.hasError" (click)="openDrawer(item)" style="cursor:pointer">
              ...
              <td class="text-muted small">{{ item.ingressClass || '-' }}</td>
              <td class="small">{{ getHosts(item) }}</td>
              <td class="font-monospace small">{{ item.loadBalancerIP || '-' }}</td>
            ...
            <tr *ngIf="filtered.length===0&&!loading"><td colspan="7" class="text-center text-muted py-4">No ingresses found</td></tr>
          </tbody>
        </table>
      </div>
    </div>
...
export class IngressComponent implements OnInit, OnDestroy {
  items:KubeIngress[]=[]; searchText=''; loading=false; error:string|null=null;
  ...
  async load(){this.loading=true;this.error=null;try{this.items=await this.k8s.getIngresses(this.ctx,this.ns)}catch(e:any){this.error=e.message}finally{this.loading=false}}
  ...
  getHosts(ing:KubeIngress):string{return ing.rules?.map(r=>r.host||'*').join(', ')||'*'}
  openDrawer(item:KubeIngress){this.drawer.open({resource:item.raw||item,resourceType:'Ingress',contextName:this.ctx,namespace:item.namespace})}
  async deleteItem(item:KubeIngress,e:Event){e.stopPropagation();if(!confirm(`Delete ingress "${item.name}"?`))return;try{await this.k8s.deleteIngress(this.ctx,item.namespace!,item.name);this.items=this.items.filter(i=>i!==item)}catch(err:any){this.error=err.message}}
}
